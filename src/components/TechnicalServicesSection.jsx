      ...
      {isOpen && (
        <>
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-[#0a0e27]/80 backdrop-blur-sm z-[100]"
          />
          <motion.div
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.9, y: 20 }}
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-2xl bg-[#0a0e27] border-2 border-[#00d9ff] rounded-2xl p-8 z-[101] shadow-[0_0_50px_rgba(0,217,255,0.2)]"
          >
            <button
              onClick={onClose}
...
const ServiceCard = ({ icon: Icon, title, description, learnMore, delay, onOpen }) => (
  <motion.div
  ...
  >
    ...
    <div className="mt-6 pt-4 border-t border-[#2a3c5f] group-hover:border-[#00d9ff]/30 transition-colors">
      <button 
        onClick={onOpen}
        className="text-[#00d9ff] text-xs font-bold uppercase tracking-widest flex items-center gap-2 group/btn"
      >
        {learnMore} <span className="transform group-hover/btn:translate-x-1 transition-transform">→</span>
      </button>
    </div>
  </motion.div>
);
